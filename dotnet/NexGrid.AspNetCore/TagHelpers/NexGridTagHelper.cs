...
//      NexGrid.createNexGrid. It waits for DOMContentLoaded when the document is
...
// copy of the script; see NexGridAssets.
...
/// <summary>
/// Renders a NexGrid: a mount point, a JSON configuration block, and the call
/// that starts the browser bundle.
/// </summary>
/// <remarks>
/// <para>Register the Tag Helpers once, in <c>_ViewImports.cshtml</c>:</para>
/// <code>@addTagHelper *, NexGrid.AspNetCore</code>
...
/// </code>
/// <para>
/// The grid handle is left on the container element as <c>element.nexgrid</c>, so
/// other scripts can call <c>refresh()</c>, <c>getSelection()</c> or
/// <c>update()</c> on it.
/// </para>
/// </remarks>
[HtmlTargetElement(TagName)]
[RestrictChildren(NexGridColumnTagHelper.TagName)]
public class NexGridTagHelper : TagHelper
{
    ...
    private const string MissingBundleMessage =
        "NexGrid: the browser bundle is not loaded. Reference " +
        NexGridAssets.ScriptPath + " from your layout before this grid renders.";
    ...
    /// the grid through <c>element.nexgrid.update(...)</c>.
    /// </summary>
    ...
    /// matched case-insensitively. Defaults to <see cref="NexGridDensity.Default"/>.
    /// </summary>
    ...
    /// case-insensitively. Defaults to <see cref="NexGridTheme.Light"/>.
    /// </summary>
    ...
    /// <c>.nxg-root</c> INSIDE the container, and this is what lands there.
    /// </summary>
    ...
    /// <c>NexGrid.createNexGrid</c> yourself — the way to add custom cell
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var columns = new NexGridColumnCollection();
        context.Items[typeof(NexGridColumnCollection)] = columns;
        await output.GetChildContentAsync().ConfigureAwait(false);

        var containerId = string.IsNullOrWhiteSpace(Id)
            ? "nxg-" + Guid.NewGuid().ToString("N", CultureInfo.InvariantCulture)[..12]
            : Id;
        ...
        output.Attributes.SetAttribute("data-nexgrid-config", configId);
        output.Content.SetHtmlContent(HtmlString.Empty);

        var json = NexGridConfigSerializer.Serialize(BuildConfig(columns));
    ...
    private NexGridConfig BuildConfig(IReadOnlyList<NexGridColumnConfig> columns) => new()
    {
        Caption = Caption ?? string.Empty,
        Columns = columns,
        Endpoint = NullIfBlank(Endpoint),
        FetchEndpoint = NullIfBlank(FetchEndpoint),
        Density = NexGridConfigSerializer.ToToken(
            NexGridAttribute.ParseEnum<NexGridDensity>(Density, TagName, "density")),
        Theme = NexGridConfigSerializer.ToToken(
            NexGridAttribute.ParseEnum<NexGridTheme>(Theme, TagName, "theme")),
    ...
    /// </summary>
    private NexGridQueryConfig? BuildQueryConfig()
    {
        var sort = SortSpec.TryParse(Sort, out var spec)
            ? new[] { new NexGridSortConfig { Field = spec.Field, Dir = spec.Direction == SortDirection.Descending ? SortSpec.DescendingToken : SortSpec.AscendingToken } }
            : Array.Empty<NexGridSortConfig>();
    ...
    {
        var container = NexGridConfigSerializer.ToJsString(containerId);
        var config = NexGridConfigSerializer.ToJsString(configId);
        var warning = NexGridConfigSerializer.ToJsString(MissingBundleMessage);
        ...
        return
            "(function(){" +
                ...
                "if(!host||!data)return;" +
                "function start(){" +
                    "var api=window." + NexGridAssets.GlobalName + ";" +
