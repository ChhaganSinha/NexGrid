// Turn a NexGridQuery plus an allowlist into one page of rows.
//
...
/// <summary>
/// Applies a <see cref="NexGridQuery"/> to an <see cref="IQueryable{T}"/> and
...
public static class NexGridQueryableExtensions
{
    ...
    /// <param name="query">The client's request, already coerced into range by <see cref="NexGridQuery.Parse(Microsoft.AspNetCore.Http.IQueryCollection)"/>.</param>
    ...
    private static NexGridQueryOptions<T> BuildOptions<T>(Action<NexGridQueryOptions<T>>? configure)
    {
        var options = new NexGridQueryOptions<T>();
    ...
    /// </summary>
    private static (int Page, int PageSize, int Skip) ResolveWindow(NexGridQuery query, int total)
    {
        ...
        NexGridQueryOptions<T> options)
    {
        ...
        var termExpression = NexGridExpressions.Parameterize(term, typeof(string));
        var nullLiteral = Expression.Constant(null, typeof(string));

        Expression? predicate = null;
        foreach (var selector in options.SearchableMembers)
        {
            var member = NexGridExpressions.Rebind(selector.Body, selector.Parameters[0], row);
            ...
            var clause = Expression.AndAlso(
                ...
                Expression.Call(member, NexGridExpressions.StringContains, termExpression));
        ...
        NexGridQueryOptions<T> options)
    {
        ...
        foreach (var entry in query.Filter)
        {
            ...
            if (!NexGridExpressions.TryConvertFilterValue(entry.Value, selector.ReturnType, out var value))
            {
                continue;
            }
            ...
            var row = Expression.Parameter(typeof(T), "row");
            var member = NexGridExpressions.Rebind(selector.Body, selector.Parameters[0], row);
            var comparison = Expression.Equal(member, NexGridExpressions.Parameterize(value, selector.ReturnType));
