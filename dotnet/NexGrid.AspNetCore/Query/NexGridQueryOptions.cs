...
// column the UI never renders. NexGrid answers that by never turning a client
...
/// <summary>
/// Declares which members of <typeparamref name="T"/> a NexGrid request is
...
/// </example>
/// </remarks>
public sealed class NexGridQueryOptions<T>
{
    ...
    /// </exception>
    public NexGridQueryOptions<T> Sortable(params Expression<Func<T, object?>>[] members)
    {
        ...
        foreach (var member in members)
        {
            ...
            if (!NexGridExpressions.TryGetMemberName(member.Body, out var name))
            {
            ...
            _sortable[name] = NexGridExpressions.Retype(member);
        }
    ...
    public NexGridQueryOptions<T> Sortable(string key, Expression<Func<T, object?>> selector)
    {
        ValidateKey(key);
        ArgumentNullException.ThrowIfNull(selector);

        _sortable[key] = NexGridExpressions.Retype(selector);
        return this;
    }
    ...
    /// </remarks>
    public NexGridQueryOptions<T> Searchable(params Expression<Func<T, string?>>[] members)
    {
        ArgumentNullException.ThrowIfNull(members);
    ...
    /// </remarks>
    public NexGridQueryOptions<T> Filterable(string key, Expression<Func<T, object?>> selector)
    {
        ValidateKey(key);
        ArgumentNullException.ThrowIfNull(selector);

        _filterable[key] = NexGridExpressions.Retype(selector);
        return this;
    }
    ...
    public NexGridQueryOptions<T> Filterable(Expression<Func<T, object?>> selector)
    {
        ArgumentNullException.ThrowIfNull(selector);

        if (!NexGridExpressions.TryGetMemberName(selector.Body, out var name))
        {
            throw new ArgumentException(
        ...
        _filterable[name] = NexGridExpressions.Retype(selector);
        return this;
    }
    ...
    /// </remarks>
    public NexGridQueryOptions<T> DefaultSort(
        Expression<Func<T, object?>> selector,
        SortDirection direction = SortDirection.Ascending)
    {
        ArgumentNullException.ThrowIfNull(selector);

        DefaultSortMember = NexGridExpressions.Retype(selector);
    ...
    private static void ValidateKey(string key)
    {
        ...
        {
            throw new ArgumentException("A NexGrid allowlist key must not be empty.", nameof(key));
        }
    }
}
