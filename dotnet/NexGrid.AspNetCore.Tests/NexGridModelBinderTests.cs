...
public class NexGridModelBinderTests
{
    [Fact]
    public async Task BindModelAsync_ExtractsQueryAndSetsModelResult()
    {
        ...
        var bindingContext = new DefaultModelBindingContext
        {
            ...
            ModelMetadata = new EmptyModelMetadataProvider().GetMetadataForType(typeof(NexGridQuery)),
        };

        var binder = new NexGridQueryModelBinder();
        await binder.BindModelAsync(bindingContext);

        Assert.True(bindingContext.Result.IsModelSet);
        var boundQuery = Assert.IsType<NexGridQuery>(bindingContext.Result.Model);
