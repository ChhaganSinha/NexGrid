...
public class TableXQueryTests
{
    ...
    {
        Assert.Equal(1, TableXQuery.Parse((IQueryCollection?)null).Page);
        Assert.Equal(1, TableXQuery.Parse(string.Empty).Page);
        Assert.Equal(1, TableXQuery.Parse("   ").Page);
    }
    ...
    public void Parse_FullQueryString_ExtractsAllComponents()
    {
        ...
        var query = TableXQuery.Parse(qs);
