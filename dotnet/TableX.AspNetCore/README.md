...
`[FromQuery] TableXQuery query` works too and binds identically — the attribute
...
| `grid-class` | string | — | Extra classes for the grid root (`.tbx-root`). The element's own `class` stays on the mount point. |
...
| Member | Description |
| --- | --- |
| `TableXQuery.Page` / `.PageSize` / `.Sort` / `.Q` / `.Filter` | The parsed request. Always in range. |
| `TableXQuery.PrimarySort` | The first sort, or `null`. |
| `TableXQuery.GetFilter(string field)` | One filter value, or `null`. Case-insensitive. |
| `TableXQuery.Parse(IQueryCollection)` | Parse from a request. |
| `TableXQuery.Parse(string)` | Parse from a raw query string. |
| `TableXQuery.BindAsync(HttpContext)` | Minimal API binding hook. Called by the framework. |
| `TableXQuery.ToQueryString()` | Render back to a query string. |
| `TableXQuery.Default` | Page 1, default size, no sort/search/filter. |
...
| `TableXQueryOptions<T>.Sortable(params Expression<Func<T, object?>>[])` | Allow sorting by these members, keyed by member name. |
| `TableXQueryOptions<T>.Sortable(string key, Expression<Func<T, object?>>)` | Allow sorting under an explicit key, or by a computed expression. |
| `TableXQueryOptions<T>.Searchable(params Expression<Func<T, string?>>[])` | Include these string members in the global search (OR'd `Contains`). |
| `TableXQueryOptions<T>.Filterable(string key, Expression<Func<T, object?>>)` | Allow `filter[key]=value` as an equality comparison. |
| `TableXQueryOptions<T>.Filterable(Expression<Func<T, object?>>)` | Same, keyed by member name. |
| `TableXQueryOptions<T>.DefaultSort(selector, direction)` | Ordering used when the request carries no usable sort. |

### Tag Helpers and assets

| Member | Description |
| --- | --- |
| `TableXTagHelper` | `<nex-grid>` |
| `TableXColumnTagHelper` | `<nex-grid-column>` |
| `TableXDensity` · `TableXTheme` · `TableXColumnAlign` | Attribute enums. |
| `TableXAssets.StylesheetPath` | `/_content/TableX.AspNetCore/tablex.css` |
| `TableXAssets.ScriptPath` | `/_content/TableX.AspNetCore/tablex.global.js` |
| `TableXAssets.ContentRoot` · `.GlobalName` | Static web asset root; the browser global (`TableX`). |
...
MIT © 2026 Chhagan Sinha. See [LICENSE](https://github.com/ChhaganSinha/TableX/blob/main/LICENSE).
